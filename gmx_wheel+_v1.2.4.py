...
def gen_wheel(results_input, projection_start, projection_end, cutoff):
  ...
  if mode == 'difference':
    projection_length = int(projection_end)-int(projection_start)
    data = get_data(results_input, 1, (projection_length+1))
  else:
  ...
  if mode == 'difference':
    colors = []
    for i in range(0, projection_length+1):
    ...
    hatches = []
    for x in range(0, projection_length+1):
